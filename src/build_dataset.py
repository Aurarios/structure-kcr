...
def generate(n: int, seed: int, augment_on: bool, workers: int, fmt: str,
             quality: int, dsf: int, min_free_gb: float) -> list[dict]:
    ...
    # split contiguous index ranges across workers
    per = n // workers
    rem = n % workers
    tasks, start = [], 0
    for w in range(workers):
        ...
        if count == 0:
            continue
        tasks.append({"worker_id": w, "start": start, "count": count, "seed": seed,
...
def write_manifests(rows: list[dict], val_frac: float) -> None:
...
def main() -> None:
    ...
    write_manifests(rows, args.val_frac)
