...
def refine_structure(lines: list[Line], page_w: float) -> list[Line]:
    ...
    for l in lines:
        ...
        if centered and tall:
            ...
            l["cls"] = _cls_id("section_header")        # short left marker (e.g. មាត្រា១)
    return lines
...
def build_markdown(blocks: list[Line]) -> str:
    ...
    while i < len(blocks):
        ...
        if name == "image":
            ...
            out.append(f"![រូបភាព](figure_{fi})" if fi is not None else "![រូបភាព]")
            i += 1; continue
        if name == "formula":
...
def build_html(blocks: list[Line]) -> str:
    ...
    while i < len(blocks):
        ...
        if name == "image":
            ...
            out.append(f'<img data-figure="{fi}" alt="figure">' if fi is not None
                       else '<div class="figure-ph">[រូបភាព]</div>')
            i += 1; continue
        if name == "formula":
        ...
        tag = {"title": f"<h1>{t}</h1>", "section_header": f"<h2>{t}</h2>",
               "list_item": f"<li>{t}</li>", "caption": f"<em>{t}</em>",
...
def assemble(lines: list[Line], page_w: float, page_h: float, merge_blocks: bool = True,
    ...
    return {
        ...
        "html": build_html(blocks),           # HTML with <table> (OCR-studio style)
